...
{
  int rank;
  MPI_Comm_rank(piston_comm, &rank);
  if(!rank) {
    va_list Argp;
...
void print(FILE* fd, const char format[],...)
{
  ...
  //MPI_Barrier(piston_comm); (This slows down the code!)
  return;
}
...
{
  int size, rank;
  MPI_Comm_size(piston_comm, &size);
  MPI_Comm_rank(piston_comm, &rank);
  ...
  //MPI_Barrier(piston_comm); //not friendly to concurrent programs
  //
  return;
}
